...
VentanaJuego::VentanaJuego()
    ...
    diagramaEstados_(700, 500, "Diagrama de estados"),
    ...
    audioArancarReproducido_(false), enPantallaInicio_(true), tamagotchi_() {
...
bool VentanaJuego::crear() {
    ...
    diagramaEstados_.obtenerVentana().setFramerateLimit(60);
    return true;
}
...
int VentanaJuego::ejecutar() {
    ...
    while (ventana_.estaAbierta()) {
        ...
        if (diagramaEstados_.estaAbierta()) {
            sf::Event eventoDiagrama{};
            while (diagramaEstados_.procesarEvento(eventoDiagrama)) {
                if (eventoDiagrama.type == sf::Event::Closed) {
                    diagramaEstados_.obtenerVentana().close();
                }
            }
        }
...
void VentanaJuego::pintarDiagramaEstados() {
    if (!diagramaEstados_.estaAbierta()) {
        return;
    }

    sf::RenderWindow& ventana = diagramaEstados_.obtenerVentana();
    ventana.clear(sf::Color(24, 29, 38));

    textoEstadoDiagrama_.setFont(fuente_);
    textoEstadoDiagrama_.setCharacterSize(25);
    textoEstadoDiagrama_.setFillColor(sf::Color::White);
    textoEstadoDiagrama_.setString(
        std::string("Estado actual: ") + tamagotchi_.ObtenerNombreEstado());
    textoEstadoDiagrama_.setPosition(25.0f, 18.0f);
    ventana.draw(textoEstadoDiagrama_);

    const std::string estadoActual = tamagotchi_.ObtenerNombreEstado();
    const sf::Vector2f posiciones[] = {
        sf::Vector2f(250.0f, 80.0f),
        sf::Vector2f(250.0f, 330.0f),
        sf::Vector2f(35.0f, 205.0f),
        sf::Vector2f(465.0f, 205.0f)
    };
    const char* nombres[] = {"Reposo", "Tuneo", "Desbielado", "Yonqueo"};

    const auto dibujarFlecha = [&ventana](sf::Vector2f inicio,
                                          sf::Vector2f fin) {
        const sf::Vector2f diferencia = fin - inicio;
        const float longitud = std::sqrt(
            diferencia.x * diferencia.x + diferencia.y * diferencia.y);
        if (longitud == 0.0f) {
            return;
        }
        const sf::Vector2f direccion = diferencia / longitud;
        const sf::Vector2f perpendicular(-direccion.y, direccion.x);
        const sf::Vector2f punta = fin - direccion * 10.0f;
        const sf::Vector2f alaIzquierda =
            punta - direccion * 12.0f + perpendicular * 6.0f;
        const sf::Vector2f alaDerecha =
            punta - direccion * 12.0f - perpendicular * 6.0f;
        sf::Vertex linea[] = {sf::Vertex(inicio, sf::Color(150, 160, 175)),
                              sf::Vertex(fin, sf::Color(150, 160, 175))};
        sf::Vertex puntaIzquierda[] = {
            sf::Vertex(punta, sf::Color(150, 160, 175)),
            sf::Vertex(alaIzquierda, sf::Color(150, 160, 175))};
        sf::Vertex puntaDerecha[] = {
            sf::Vertex(punta, sf::Color(150, 160, 175)),
            sf::Vertex(alaDerecha, sf::Color(150, 160, 175))};
        ventana.draw(linea, 2, sf::Lines);
        ventana.draw(puntaIzquierda, 2, sf::Lines);
        ventana.draw(puntaDerecha, 2, sf::Lines);
    };

    dibujarFlecha(sf::Vector2f(320.0f, 140.0f), sf::Vector2f(320.0f, 330.0f));
    dibujarFlecha(sf::Vector2f(250.0f, 375.0f), sf::Vector2f(145.0f, 280.0f));
    dibujarFlecha(sf::Vector2f(465.0f, 245.0f), sf::Vector2f(395.0f, 245.0f));
    dibujarFlecha(sf::Vector2f(145.0f, 205.0f), sf::Vector2f(250.0f, 115.0f));

    for (int indice = 0; indice < 4; ++indice) {
        sf::RectangleShape estado(sf::Vector2f(180.0f, 70.0f));
        estado.setPosition(posiciones[indice]);
        estado.setFillColor(estadoActual == nombres[indice]
                                ? sf::Color(47, 139, 87)
                                : sf::Color(53, 64, 80));
        estado.setOutlineThickness(3.0f);
        estado.setOutlineColor(sf::Color(205, 214, 226));
        ventana.draw(estado);

        sf::Text nombre;
        nombre.setFont(fuente_);
        nombre.setCharacterSize(22);
        nombre.setFillColor(sf::Color::White);
        nombre.setString(nombres[indice]);
        const sf::FloatRect limites = nombre.getLocalBounds();
        nombre.setPosition(
            posiciones[indice].x + (180.0f - limites.width) / 2.0f - limites.left,
            posiciones[indice].y + (70.0f - limites.height) / 2.0f - limites.top);
        ventana.draw(nombre);
    }
